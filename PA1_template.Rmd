...
```{r}
qplot(TotSteps, data=DailySteps,xlab="number of steps", bins=40, ylab="frequency", main="Total number of steps per day")
```
...
plot(IntervalSteps$interval, IntervalSteps$MeanSteps, type = "l", xlab = "5-minute interval", ylab = "Average number of steps", main ="Average number of steps taken in a 5-minute interval")
```
...
```{r}
qplot(TotSteps, data=DailyStepsF,xlab="number of steps", bins=40, ylab="frequency", main="Total number of steps per day for filled in dataset")
```
...
corrected_data$weekend <-as.factor(ifelse(corrected_data$DayOfWeek=="Saturday" | corrected_data$DayOfWeek=="Sunday","weekend","weekday"))
```
#####2. Make a panel plot containing a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis). See the README file in the GitHub repository to see an example of what this plot should look like using simulated data.
```{r, warning=FALSE, message=FALSE}
  MeanStepsWeekdayType <- corrected_data %>% group_by(interval,weekend) %>%   summarize(MeanSteps = mean(steps))
  qplot(interval,MeanSteps,data= MeanStepsWeekdayType,geom="line",facets=weekend~.,xlab="5-minute interval",ylab="average number of steps",main="Comparison of average number of steps patterns on Weekdays and Weekends")
```